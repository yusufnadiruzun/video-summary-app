...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [notifSettings] = await db.execute(
      `SELECT notification_email, telegram_chat_id FROM notifications WHERE user_id = ?`,
      [userId],
    );
   ...
   WHERE user_id = ? ORDER BY created_at DESC LIMIT 20`,
    ...
    res.status(200).json({
      ...
      data: {
        ...
        startDate: userPkg.Start_Date
          ? new Date(userPkg.Start_Date).toLocaleDateString("en-US")
          : null,
        endDate:
          userPkg.End_Date && userPkg.End_Date !== "Süresiz"
            ? new Date(userPkg.End_Date).toLocaleDateString("en-US")
            : "Unlimited",
        notifications: {
        ...
        activeChannels: subscriptions
          .filter(
            (s) =>
              s.channel_id &&
              !["default", "default_channel", ""].includes(s.channel_id),
          )
